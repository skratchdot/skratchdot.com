...
Another thing about the code we copied: we can ignore the $post*count variable (since we are \_always* showing post counts). We also want to make sure the post count shows up inside our anchor tag. To accomplish that, we can change:
